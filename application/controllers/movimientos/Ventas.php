...
class Ventas extends CI_Controller {
	...
	public function store(){
		...
		$nro_comprobante = $this->numberDocumentGenerated($comprobante_id);
	...
	public function savecliente(){
		...
		//$nro_comprobante = $this->input->post("numero");

		$data  = array(
			'nombre' => $nombre, 
		//	'tipo_documento_id' => $tipodocumento,
		//	'tipo_id_cliente' => $tipocliente,
			'direccion' => $direccion,
			'telefono' => $telefono,
		//	'nro_comprobante' => $nro_comprobante,
			'estado' => "1"
	...
	protected function retornarStockVenta($idventa){
        ...
        foreach ($detalles as $detalle) {
            $infoproducto = $this->Productos_model->getProducto($detalle->id_producto);
            //reponer stock de los productos asociados
            $productosAsociados = $this->Productos_model->getProductosA($detalle->id_producto);
            ...
            $this->Productos_model->update($detalle->id_producto,$dataProducto);
        }
	}
